...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    const [ru, rt, re, rb, rpe, rer, radv] = await Promise.allSettled([
      ...
      fetchPontosExtras(), fetchExtraRules(), fetchAdvertencias(),
    ]);
        ...
        if (session) {
          ...
          if (!profile && session.email) {
            try {
              ...
              profile = us.find(u => u.email === session.email) || null;
            } catch(e) { console.warn("[init] fetch profile failed"); }
          }
          if (!profile && session.email) {
            const LOCAL = [
              { id:"u-vivian", email:"vivian@orioncloudkitchens.com.br", name:"Vivian", role:"admin", nivel:"admin", cargo:"Gestao", setor:"Gestao", avatar:"VI", ativo:true, elegivel_bonus:true },
              ...
              { id:"u-eduardo", email:"eduardo@orion.com.br", name:"Eduardo", role:"colaborador", nivel:"lider", cargo:"Lider", setor:"Operacional", avatar:"ED", ativo:true, elegivel_bonus:true },
              { id:"u-adala", email:"adala@orion.com.br", name:"Adala", role:"colaborador", nivel:"lider", cargo:"Lider", setor:"Operacional", avatar:"AD", ativo:true, elegivel_bonus:true },
  ...
  const handleSetUsers = useCallback(async (updated, userToSave) => {
    setUsers(updated);
    if (userToSave) {
      try { await upsertUser(userToSave); }
      catch(e) {
  ...
  const handleSetTasks = useCallback(async (updated, taskToSave) => {
    setTasks(updated);
    if (taskToSave) {
      try { await upsertTask(taskToSave); }
      catch(e) {
  ...
  // SEM fetchExecucoes apos salvar — UI ja foi atualizada otimisticamente
  const handleSetExecutions = useCallback(async (updated, execToSave) => {
    ...
    if (execToSave) {
      try {
        ...
        toast("Erro ao salvar execucao: " + (e?.message ?? e), "error");
        setExecutions(prev => prev.filter(ex => ex.id !== execToSave.id));
      }
    }
  }, [toast]);
  ...
  const handleSetBonusRules = useCallback(async (rules) => {
    setBonusRules(rules);
    try { await saveBonusRules(rules); }
    catch(e) { toast("Erro ao salvar configuracoes: " + (e?.message ?? e), "error"); }
  }, [toast]);
  ...
  const shared = {
    users, tasks, executions, bonusRules, user, toast, reloadAll,
    ...
    removeUser: async (id) => {
      try { await deleteUser(id); setUsers(prev => prev.filter(u => u.id !== id)); toast("Colaborador removido"); }
      catch(e) { toast("Erro ao remover: " + e.message, "error"); }
    },
    pontosExtras,
    addPontosExtras: handleAddPontosExtras,
    removePontosExtras: async (id) => {
      try { await deletePontosExtras(id); setPontosExtras(p => p.filter(e => e.id !== id)); toast("Registro removido"); }
      catch(e) { toast("Erro ao remover: " + e.message, "error"); }
    },
    extraRules,
    ...
    addAdvertencia: async (entry) => {
      try { const saved = await insertAdvertencia(entry); setAdvertencias(p => [saved, ...p]); return saved; }
      catch(e) { toast("Erro ao salvar advertencia: " + e.message, "error"); throw e; }
    },
    removeAdvertencia: async (id) => {
      try { await deleteAdvertencia(id); setAdvertencias(p => p.filter(a => a.id !== id)); toast("Advertencia removida"); }
        ...
        <main className="main-content" style={{ flex:1, padding:"32px 36px", overflowY:"auto", minHeight:"100vh", maxHeight:"100vh" }}>
          {!USE_SUPABASE && (
            <div style={{ display:"flex", alignItems:"center", gap:10, background:"#fffbeb", border:"1px solid #fde68a", borderRadius:12, padding:"10px 14px", marginBottom:20, fontSize:12 }}>
              ...
              <div>
                <strong style={{ color:"#92400e" }}>Dados locais — sem sincronizacao</strong>
                <span style={{ color:"#b45309", marginLeft:6 }}>Mobile e desktop tem bases separadas.</span>
              </div>
            </div>
          )}
          {pages[active] || <div style={{ color:"#94a3b8", padding:40 }}>Selecione uma opcao no menu</div>}
        </main>
      </div>
